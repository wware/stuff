...
{setvar(cintern("*slib-version*"),
	cintern("$Id: slib.c,v 1.29 1996/06/12 12:35:45 gjc Exp $"),
	NIL);}

char *siod_version(void)
{return("3.2 12-JUN-96");}
 ...
 switch TYPE(fcn)
   {case tc_subr_0:
    ...
    default:
      ...
      if (p->leval)
	err("have eval, dont know apply",fcn);
      else
	err("cannot be applied",fcn);}}
...
LISP fopen_cg(FILE *(*fcn)(char *,char *),char *name,char *how)
{LISP sym;
 long flag;
 char errmsg[80];
 flag = no_interrupt(1);
