...
/**
 * CDCInitEP initializes CDC endpoints, buffer descriptors, internal
 * state-machine, and variables. It should be called after the USB
 * host has sent out a SET_CONFIGURATION request. See
 * USBStdSetCfgHandler() in usb9.c for examples.
 */
void CDCInitEP(void)
{
...
/**
 * Copy a string of bytes received through USB CDC Bulk OUT endpoint
 * to a user's specified location. This is a non-blocking function. It
 * does not wait for data if there is no data available. Instead it
 * returns '0' to notify the caller that there is no data available.<P>
 *
 * Pre-condition: Value of input argument 'len' should be smaller than
 * the maximum endpoint size responsible for receiving bulk data from
 * USB host for CDC class. Input argument 'buffer' should point to a
 * buffer area that is bigger or equal to the size specified by 'len'.<p>
 *
 * buffer: Pointer to where received bytes are to be stored<p>
 * len: The number of bytes expected.<p>
 * Output:          The number of bytes copied to buffer.<p>
 *
 * Side Effects: Publicly accessible variable cdc_rx_len is updated
 * with the number of bytes copied to buffer. Once getsUSBUSART is
 * called, subsequent retrieval of cdc_rx_len can be done by calling
 * macro mCDCGetRxLength().<p>
 *
 * Caveat: If the actual number of bytes received is larger than the
 * number of bytes expected (len), only the expected number of bytes
 * specified will be copied to buffer. If the actual number of bytes
 * received is smaller than the number of bytes expected (len), only
 * the actual number of bytes received will be copied to buffer.
 */
...
/**
 * Write a string of data to the USB including the null character. Use
 * this version, 'puts', to transfer data located
 * in data memory.<p> Pre-condition: cdc_trf_state must be in the
 * CDC_TX_READY state.<p> The string of characters pointed to by
 * 'data' must not exceed 255 bytes.<p>
 *
 * 'data' is a pointer to a null-terminated string of data.<p>
 *
 * Note: The transfer mechanism for device-to-host(put) is more
 * flexible than host-to-device(get). It can handle a string of data
 * larger than the maximum size of bulk IN endpoint. A state machine
 * is used to transfer a long string of data over multiple USB
 * transactions. See CDCTxService() for more details.
 */
...
/**
 * Write a string of data to the USB including the null character. Use
 * this version, 'putrs', to transfer data literals and data located
 * in program memory.<p> Pre-condition: cdc_trf_state must be in the
 * CDC_TX_READY state.<p> The string of characters pointed to by
 * 'data' must not exceed 255 bytes.<p>
 *
 * 'data' is a pointer to a null-terminated string of data.<p>
...
/**
 * Handle device-to-host transaction(s). This function should be
 * called once per Main Program loop.
 */
