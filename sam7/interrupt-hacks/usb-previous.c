...
static uint AT91F_CDC_Read(AT91PS_USBDEV pUsbdev, char *pData,
			   uint length);
static uint AT91F_CDC_Write(AT91PS_USBDEV pUsbdev, const char *pData,
			    uint length);
...
//* \brief
//*----------------------------------------------------------------------------
AT91PS_USBDEV AT91F_USBDEV_Open(AT91PS_USBDEV pUsbdev, AT91PS_UDP pUdp,
                                                              ...
                                                              ushort wValue, ushort wIndex))
{
    ...
    pUsbdev->setConfiguration = setConfiguration;
    pUsbdev->clearFeatureEndpoint = clearFeatureEndpoint;
    return pUsbdev;
}
...
//*----------------------------------------------------------------------------
static void AT91F_HID_SendReport(AT91PS_USBDEV pHid, char *report,
				 int reportSize)
{
...
//*----------------------------------------------------------------------------
static void AT91F_USB_SendData(AT91PS_UDP pUdp, const char *pData,
			       uint length)
{
    uint cpt = 0;
    AT91_REG csr;
...
static void setup_packet_handler(AT91PS_USBDEV pUsbdev)
{
	...
	pUDP->UDP_GLBSTATE = (wValue) ? AT91C_UDP_CONFG : AT91C_UDP_FADDEN;
        (pUsbdev->setConfiguration)(pUsbdev, wValue);
	break;
	...
	wIndex &= 0x0F;
	if ((wValue == 0) && wIndex && (wIndex <= 3)) {
            (pUsbdev->clearFeatureEndpoint)(pUsbdev, wValue, wIndex);
	    AT91F_USB_SendZlp(pUDP);
	} else
	    AT91F_USB_SendStall(pUDP);
	break;
