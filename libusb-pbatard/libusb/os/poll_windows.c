 ...
 * Return a pollable file descriptor struct, or INVALID_WINFD on error.<p>
 *
...
struct winfd usbi_create_fd(HANDLE handle, int access_mode)
{
	...
	if ((handle == 0) || (handle == INVALID_HANDLE_VALUE)) {
		usbi_dbg("having trouble at line %d", __LINE__);
		...
		if (poll_fd[i].fd < 0) {
			...
			if (!CancelIoEx_Available) {
				...
				if (!DuplicateHandle(GetCurrentProcess(), handle, GetCurrentProcess(),
					...
					usbi_dbg("successfully duplicated handle");
					_poll_fd[i].original_handle = handle;
				}
			} else {
				usbi_dbg("setting wfd.handle to first argument");
				wfd.handle = handle;
			}
			...
			SetEvent(fd_update);
#endif
			usbi_dbg("successfully completed at line %d", __LINE__);
			return wfd;
		}
	}
