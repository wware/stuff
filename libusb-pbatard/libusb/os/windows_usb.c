...
/**
 * enumerate interfaces for a specific GUID<p>
 *
 * Parameters<ul>
 * <li>dev_info: a pointer to a dev_info list
 * <li>dev_info_data: a pointer to an SP_DEVINFO_DATA to be filled (or NULL if not needed)
 * <li>guid: the GUID for which to retrieve interface details
 * <li>index: zero based index of the interface in the device info list
 * </ul>
 *
...
/**
 * init: libusb backend init function<p>
 *
...
/**
 * HCD (root) hubs need to have their device descriptor manually populated<p>
 *
...
static int winusb_claim_interface(struct libusb_device_handle *dev_handle, int iface)
{
	...
	if ((is_using_usbccgp) || (iface == 0)) {
		...
		usbi_dbg("assigning handle_priv->interface_handle[iface].api_handle to %d at line %d", winusb_handle, __LINE__);
		...
		if ((winusb_handle == 0) || (winusb_handle == INVALID_HANDLE_VALUE)) {
			...
			if (WinUsb_Initialize(file_handle, &winusb_handle)) {
				usbi_dbg("assigning handle_priv->interface_handle[0].api_handle to %d at line %d",
					winusb_handle, __LINE__);
		...
		if (!WinUsb_GetAssociatedInterface(winusb_handle, (UCHAR)(iface-1),
			...
			usbi_dbg("assigning handle_priv->interface_handle[iface].api_handle to INVALID_HANDLE_VALUE at line %d",
				__LINE__);
			handle_priv->interface_handle[iface].api_handle = INVALID_HANDLE_VALUE;
			switch(GetLastError()) {
...
static int winusb_release_interface(struct libusb_device_handle *dev_handle, int iface)
{
	...
	usbi_dbg("assigning handle_priv->interface_handle[iface].api_handle to INVALID_HANDLE_VALUE at line %d", __LINE__);
	handle_priv->interface_handle[iface].api_handle = INVALID_HANDLE_VALUE;
	...
	for (i = 0; i < USB_MAXINTERFACES; i++) {
		if ( (priv->usb_interface[i].path != NULL)
		  && (priv->usb_interface[i].apib->id == USB_API_HID) ) {
			...
			usbi_dbg("assigning handle_priv->interface_handle[iface].api_handle to %d at line %d",
				hid_handle, __LINE__);
...
static int hid_submit_control_transfer(struct usbi_transfer *itransfer)
{
	...
	usbi_dbg("hid_handle = %d", hid_handle);
	// Always use the handle returned from usbi_create_fd (wfd.handle)
